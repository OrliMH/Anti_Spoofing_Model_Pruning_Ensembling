import sys
sys.path.append("/home/disk2/live_detect/week3/week3code-CVPR19-Face-Anti-spoofing")
# print(sys.path)
from utils import *
...
class FDDataset(Dataset):
    ...
    def set_mode(self, mode, fold_index):
        ...
        if self.mode == 'test':
            ...
            if self.balance:
                self.train_list = transform_balance(self.train_list) # [positive list, negative list] positive is real neg is fake
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            if self.balance:
                if random.randint(0,1)==0:
                    tmp_list = self.train_list[0] # positive
                else:
        ...
        # print("after read, img size:{}".format(color.shape))
        color = cv2.resize(color,(RESIZE_SIZE,RESIZE_SIZE)) # 112
        depth = cv2.resize(depth,(RESIZE_SIZE,RESIZE_SIZE))
        ir = cv2.resize(ir,(RESIZE_SIZE,RESIZE_SIZE))
        # print("after resize, img size:{}".format(color.shape))
