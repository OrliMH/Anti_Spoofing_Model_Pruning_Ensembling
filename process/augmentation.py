...
def random_cropping(image, target_shape=(32, 32, 3), is_random = True): # target_shape传进来128
    image = cv2.resize(image,(RESIZE_SIZE,RESIZE_SIZE)) # 112
    target_h, target_w,_ = target_shape # 128
    height, width, _ = image.shape # 112
...
def TTA_36_cropps(image, target_shape=(32, 32, 3)): # 返回固定位置的36个
    image = cv2.resize(image, (RESIZE_SIZE, RESIZE_SIZE))
...
def random_resize(img, probability = 0.5,  minRatio = 0.2):
    ...
    ratio = random.uniform(minRatio, 1.0) # target ratio
...
def color_augumentor(image, target_shape=(32, 32, 3), is_infer=False):# target_shape传进来128
    if is_infer:
        ...
        image = augment_img.augment_image(image)
        image = random_resize(image) # w,h -> w*r,h*r -> w,h r=[0.2, 1] 112
        image = random_cropping(image, target_shape, is_random=True)  # target_shape传进来128
        return image
